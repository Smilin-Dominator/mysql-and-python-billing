...
class master_bill(object):
    ...
    def bill_write(self) -> None:
        master_bill_path = path.join(self.var_path + '/master_bill.txt')
    ...
    def bill_collect(self):
        temp_path = path.join(self.var_path + '/' + self.bill)
...
def delete_previous(start: str):
    hits = glob(f"./sales_reports/sales-report_from_{start}*")
...
def main():
    ...
    key = input_screen()
    if key == 'T':
        var_time = strftime("%d_of_%B")
        var_path = f'./bills/{var_time}/'
        ls_l = listdir(var_path)
        for bill in ls_l:
        ...
        for directory in multiverse:
            var_path = path.join('./bills/', directory)
            ls_l = listdir(var_path)
            for bill in ls_l:
                if bill.startswith("[BILL]"):
                    to_write = master_bill(var_path=var_path, bill=bill).bill_collect()
